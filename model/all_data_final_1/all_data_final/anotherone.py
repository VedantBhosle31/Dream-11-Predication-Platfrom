...
merged['cost'] = 7 + (6 * (merged['matchpoints_avg'] - min_matchpoints) // (max_matchpoints - min_matchpoints))

# Assign random integer score between 7 and 9 to missing 'matchpoints_avg'
merged.loc[merged['matchpoints_avg'].isna(), 'cost'] = np.random.randint(6,9, size=merged['matchpoints_avg'].isna().sum())
