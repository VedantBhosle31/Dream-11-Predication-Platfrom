...
class PerformancePredictor:
    def __init__(self, data_paths, target_columns_dict, feature_columns_dict):
        ...
        self.target_columns_dict = target_columns_dict  
    ...
    def preprocess_data(self, target_column, data,train_from,train_to,test_from,test_to):
        ...
        data_train=data[(pd.to_datetime(data['date']) < datetime.fromisoformat(train_to)) &
                  (pd.to_datetime(data['date']) > datetime.fromisoformat(train_from))]#$$$

        data_test=data[(pd.to_datetime(data['date']) < datetime.fromisoformat(test_to)) &
                  (pd.to_datetime(data['date']) > datetime.fromisoformat(test_from))]#$$$



        if data_train.empty or data_test.empty:
            print(f"Skipping target '{target_column}' as no matches found in the specified date range.")
    ...
    def train_and_evaluate_model(self, model_type, data,target_column,train_from,train_to,test_from,test_to):

        """Train and evaluate a model without optimization"""
        passed,X_train, X_test, y_train, y_test=self.preprocess_data(target_column, data,train_from,train_to,test_from,test_to)
        if( not passed):
    ...
    def save_all_models(self, train_to):
        """Save all models into a single .pkl file"""
        big_pickle = {}
        big_pickle['scalers'] = self.scalers
        big_pickle['models'] = self.models
        
        location = os.path.join('model_artifacts')
        
        with open(os.path.join(location, 'model_' + train_to + '.pkl'), 'wb') as f:
            pickle.dump(big_pickle, f)

    def run_training(self,train_from,train_to,test_from,test_to,model_type='xgboost'):
        ...
        for player_type, data_path in self.data_paths.items():
            ...
            for target_column in target_columns:
                ...
                passed,metrics, model, X_test, y_test, y_pred = self.train_and_evaluate_model(model_type, data, target_column,train_from,train_to,test_from,test_to)

                data_test=data[(pd.to_datetime(data['date']) < datetime.fromisoformat(test_to)) & (pd.to_datetime(data['date']) > datetime.fromisoformat(test_from))]#$$$
                ...
                if passed:
                  for true_value, predicted_value,player_id,m_id,ball,name_1,date in zip(y_test, y_pred,t_ID,match_id,balls,name,data_test['date']):
                      ...
                      if m_id not in mapping:
                        if 'team' in data_test.columns:
                            ...
                            team_1 = data_test.loc[data['match_id']==m_id]['team_name']
                        try:
                            team_2 = data_test.loc[data['match_id']==m_id]['opposition_name']
                            mapping[m_id] = [team_1, team_2,date]
                        except:
                          print(f"ERROR TEAMS NOT FOUND{m_id}")

        os.makedirs('pickles', exist_ok=True)
        self.save_all_models(train_to)

        predictions_df = pd.DataFrame(all_predictions)

        temp_location = os.path.join('data','interim','predictions_output.csv')
        predictions_df.to_csv(temp_location, index=False)
        print(f"Interim Predictions done")
...
def calculate_batsman_points_t20(runs, boundaries, sixes, strike_rate, balls_faced):
    ...
    batsman_points+=np.where((balls_faced >= 10) & (runs_per_100_balls > 170),6,0)   # Above 170 runs per 100 balls
        # elif 150 < runs_per_100_balls <= 170:
    batsman_points+=np.where((balls_faced >= 10) & (runs_per_100_balls <= 170) & (runs_per_100_balls>150),4,0)  # Between 150.01 and 170 runs per 100 balls
        # elif 130 <= runs_per_100_balls <= 150:
    batsman_points+=np.where((balls_faced >= 10) & (runs_per_100_balls <= 150) & (runs_per_100_balls>130),2,0)    # Between 130 and 150 runs per 100 balls
        # elif 60 <= runs_per_100_balls <= 70:
    batsman_points+=np.where((balls_faced >= 10) & (runs_per_100_balls <= 70) & (runs_per_100_balls>60),-2,0)    # Between 60 and 70 runs per 100 balls
        # elif 50 <= runs_per_100_balls < 60:
    batsman_points+=np.where((balls_faced >= 10) & (runs_per_100_balls <= 60) & (runs_per_100_balls>50),-4,0)   # Between 50 and 59.99 runs per 100 balls
        # elif runs_per_100_balls < 50:
    batsman_points+=np.where((balls_faced >= 10) & (runs_per_100_balls <50),-4,0)   # Below 50 runs per 100 balls
...
def calculate_batsman_points_odi(runs, boundaries, sixes, strike_rate, balls_faced):
    ...
    batsman_points += np.where((balls_faced >= 20) & (runs_per_100_balls < 30),-6,0)  # Below 30 runs per 100 ball
    return batsman_points
...
def calculate_bowler_points_t20(wickets, bowled_lbw, maidens, economy, balls):
    ...
    # if balls >= 12:
    #     if economy < 5:
    bowler_points+=np.where((balls >= 12) & (economy < 5),6,0)   # Below 5 runs per over
        # elif 5 <= economy < 6:
    bowler_points+=np.where((balls >= 12) & (economy>=5) & (economy<6),4,0)  # Between 5 and 5.99 runs per over
        # elif 6 <= economy <= 7:
    bowler_points+=np.where((balls >= 12) & (economy>=6) & (economy<7),2,0)   # Between 6 and 7 runs per over
        # elif 10 <= economy <= 11:
    bowler_points+=np.where((balls >= 12) & (economy>=10) & (economy<=11),-2,0)  # Between 10 and 11 runs per over
        # elif 11 < economy <= 12:
    bowler_points+=np.where((balls >= 12) & (economy>11) & (economy<=12),-4,0)   # Between 11.01 and 12 runs per over
        # elif economy > 12:
    bowler_points+=np.where((balls >= 12) & (economy > 12),-6,0)   # Above 12 runs per over
...
def calculate_fielder_points_test(catches, stumpings):
    ...
    return fielder_points
def points_calculator(format, runs, boundaries, sixes, strike_rate, wickets, bowled_lbw, maidens, economy, catches, stumpings, balls_bowled=30):#assume each bowler bowls 2 overs
    ...
    if format[-2:] == "20":
        ...
        total_points = (calculate_batsman_points_odi(runs, boundaries, sixes, strike_rate, balls_faced_while_batting) +
                        calculate_bowler_points_odi(wickets, bowled_lbw, maidens, economy, balls_bowled) +
                        calculate_fielder_points_odi(catches, stumpings))

    else:
        total_points = (calculate_batsman_points_test(runs, boundaries, sixes, strike_rate, balls_faced_while_batting) +
                        calculate_bowler_points_test(wickets, bowled_lbw, maidens, economy, balls_bowled) +
...
data_paths = {
    'ODI_batting':os.path.join(path, 'ODI', 'batter.csv'),
    'ODI_bowling':os.path.join(path, 'ODI', 'bowler.csv'),
    'ODI_fielding': os.path.join(path, 'ODI', 'fielder.csv'),
    
    'ODM_batting':os.path.join(path, 'ODM', 'batter.csv'),
    'ODM_bowling':os.path.join(path, 'ODM', 'bowler.csv'),
    'ODM_fielding': os.path.join(path, 'ODM', 'fielder.csv'),
    
    'T20_batting':os.path.join(path, 'T20', 'batter.csv'),
    'T20_bowling':os.path.join(path, 'T20', 'bowler.csv'),
    'T20_fielding': os.path.join(path, 'T20', 'fielder.csv'),
    
    'IT20_batting':os.path.join(path, 'IT20', 'batter.csv'),
    'IT20_bowling':os.path.join(path, 'IT20', 'bowler.csv'),
    'IT20_fielding': os.path.join(path, 'IT20', 'fielder.csv'),
    
    'Test_batting':os.path.join(path, 'Test', 'batter.csv'),
    'Test_bowling':os.path.join(path, 'Test', 'bowler.csv'),
    'Test_fielding': os.path.join(path, 'Test', 'fielder.csv'),
    
    'MDM_batting':os.path.join(path, 'MDM', 'batter.csv'),
    'MDM_bowling':os.path.join(path, 'MDM', 'bowler.csv'),
    'MDM_fielding': os.path.join(path, 'MDM', 'fielder.csv'),
    
    'WODI_batting':os.path.join(path, 'WODI', 'batter.csv'),
    'WODI_bowling':os.path.join(path, 'WODI', 'bowler.csv'),
    'WODI_fielding': os.path.join(path, 'WODI', 'fielder.csv'),
    
    'WT20_batting':os.path.join(path, 'WT20', 'batter.csv'),
    'WT20_bowling':os.path.join(path, 'WT20', 'bowler.csv'),
    'WT20_fielding': os.path.join(path, 'WT20', 'fielder.csv'),
    
    'WTest_batting':os.path.join(path, 'WTest', 'batter.csv'),
    'WTest_bowling':os.path.join(path, 'WTest', 'bowler.csv'),
    'WTest_fielding': os.path.join(path, 'WTest', 'fielder.csv'),
    
    'WODM_batting':os.path.join(path, 'WODM', 'batter.csv'),
    'WODM_bowling':os.path.join(path, 'WODM', 'bowler.csv'),
    'WODM_fielding': os.path.join(path, 'WODM', 'fielder.csv'),
    
    'WIT20_batting':os.path.join(path, 'WIT20', 'batter.csv'),
    'WIT20_bowling':os.path.join(path, 'WIT20', 'bowler.csv'),
    'WIT20_fielding': os.path.join(path, 'WIT20', 'fielder.csv')
}
...
if __name__ == "__main__":
    ...
    model.run_training(*date)#/content/drive/MyDrive/all_data_final/all_data_final/IT20/batter.csv



    temp_location = os.path.join('data','interim','predictions_output.csv')
    output=pd.read_csv(temp_location)

    lst_feature={'runs':'runs','6':'sixes','4':'boundaries','strike':'strike_rate','stump':'stumpings','lbw':'bowled_lbw','catch':'catches','econ':'economy','wicket':'wickets','maiden':'maidens'}
    ...
    for i in list(match_groups.groups.keys()):
        ...
        format=[i for i in list(df1['target_column'].unique()) if 'run' in i][0].split('_')[0]
        ...
        dfx['pred_points']=points_calculator(format=format, runs=dfx['runs'], boundaries=dfx['boundaries'], sixes=dfx['sixes'], strike_rate=dfx['strike_rate'], wickets=dfx['wickets'], bowled_lbw=dfx['bowled_lbw'], maidens=dfx['maidens'], economy=dfx['economy'], catches=dfx['catches'], stumpings=dfx['stumpings'],balls_bowled=12)
        dfx['real_points']=points_calculator(format=format, runs=dfr['runs'], boundaries=dfr['boundaries'], sixes=dfr['sixes'], strike_rate=dfr['strike_rate'], wickets=dfr['wickets'], bowled_lbw=dfr['bowled_lbw'], maidens=dfr['maidens'], economy=dfr['economy'], catches=dfr['catches'], stumpings=dfr['stumpings'],balls_bowled=12)
        ...
        player_df_real=pd.concat([player_df_real,pd.DataFrame({'player'+str(i)+'_real':df_real.iloc[i]['name'] for i in range(11)},index=[i])])
        player_df_predicted=pd.concat([player_df_predicted,pd.DataFrame({'player'+str(i):df_pred.iloc[i]['name']  for i in range(11)},index=[i])])
        points_df_real=pd.concat([points_df_real,pd.DataFrame({'player'+str(i)+'real_points':df_real.iloc[i]['real_points'] for i in range(11)},index=[i])])
        points_df_predicted=pd.concat([points_df_predicted,pd.DataFrame({'player'+str(i)+'predicted_points':df_pred.iloc[i]['pred_points'] for i in range(11)},index=[i])])



    ####final prediction area
    final_predictions = pd.DataFrame(columns=['index',
                'Match Date','Team','Team 2',
                'Predicted Player 1','Predicted Player 1 Points','Predicted Player 2','Predicted Player 2 Points','Predicted Player 3','Predicted Player 3 Points','Predicted Player 4','Predicted Player 4 Points','Predicted Player 5','Predicted Player 5 Points','Predicted Player 6','Predicted Player 6 Points','Predicted Player 7','Predicted Player 7 Points','Predicted Player 8','Predicted Player 8 Points','Predicted Player 9','Predicted Player 9 Points','Predicted Player 10','Predicted Player 10 Points','Predicted Player 11','Predicted Player 11 Points',
                'Dream Team Player 1','Dream Team Player 1 Points','Dream Team Player 2','Dream Team Player 2 Points','Dream Team Player 3','Dream Team Player 3 Points','Dream Team Player 4','Dream Team Player 4 Points','Dream Team Player 5','Dream Team Player 5 Points','Dream Team Player 6','Dream Team Player 6 Points','Dream Team Player 7','Dream Team Player 7 Points','Dream Team Player 8','Dream Team Player 8 Points','Dream Team Player 9','Dream Team Player 9 Points','Dream Team Player 10','Dream Team Player 10 Points','Dream Team Player 11','Dream Team Player 11 Points',
                'Total Points Predicted','Total Dream Team Points', 'Total points MAE'
            ])
    ...
    for i in points_df_real.index:
        for k in range(11):
            ...
            final_predictions.loc[i, pred + ' Points'] = points_df_predicted[pred_col+'predicted_points'][i]
            final_predictions.loc[i, real] = player_df_real[real_col][i]
            final_predictions.loc[i, real + ' Points'] =  points_df_real[pred_col+'real_points'][i]
    ...
    final_predictions['Total Points Predicted']=final_predictions[predlist].sum(axis=1)
    final_predictions['Total Dream Team Points']=final_predictions[realist].sum(axis=1)
    final_predictions['Total points MAE']=abs(final_predictions['Total Points Predicted']-final_predictions['Total Dream Team Points'])
    
    output_path = os.path.join('data','output_files','final_result.csv')
