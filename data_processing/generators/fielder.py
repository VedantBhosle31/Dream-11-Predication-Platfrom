...
def func(gender, location, formats, matchid_mapping):
    location_runouts = os.path.join('data','raw','additional_data','runouts_with_cricsheet_id.csv')
    ...
    for format in formats:
        ...
        overall_fielding_details = pd.DataFrame(columns=['caught','runouts','stumped','innings']
                                                )

        player_fielding = pd.DataFrame(columns=['date','player_name','player_id','venue_name','previous_innings_fielded','match_id',
                                                'match_catches','match_runouts','match_stumpings','match_fantasy_points',
                                                'previous_catches','previous_runouts','previous_stumpings',
                                                'pFa_catches','pFa_stumpings','pFa_runouts',
                                                'venue_catches','venue_runouts','venue_stumpings','venue_innings']
                                    )
        ...
        for ele in date_file_pair:
            ...
            for player, outs in wickets.items():
                ...
                player_fielding.loc[count,'match_fantasy_points'] = 8*player_fielding.loc[count,'match_catches'] + 12*player_fielding.loc[count,'match_stumpings'] + (4 if player_fielding.loc[count,'match_catches']>=3 else 0) + round((12*direct_runouts/total_runouts + 6*indirect_runouts/total_runouts)*player_fielding.loc[count,'match_runouts'])
                ...
                player_fielding.loc[count,'venue_innings'] = venue_dict[venue]['innings']
                player_fielding.loc[count,'venue_catches'] = venue_dict[venue]['caught']
                player_fielding.loc[count,'venue_runouts'] = venue_dict[venue]['run_out']
                player_fielding.loc[count,'venue_stumpings'] = venue_dict[venue]['stumped']
        ...
        for i in range(len(player_fielding)):
            ...
            if not matched.empty:
                player_fielding.iloc[i, player_fielding.columns.get_loc('player_id')] = matched['identifier'].values[0]
            else:
                # print(f"No matching entry for {name}: {len(mapping[mapping['unique_name'] == name])} matches")
                player_fielding.iloc[i, player_fielding.columns.get_loc('player_id')] = 'xxxxxxxx'
