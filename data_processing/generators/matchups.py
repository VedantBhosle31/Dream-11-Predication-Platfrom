    ...
    for ball in innings1:
        for delivery in ball:
            ...
            if (batsman, bowler) not in matchup:
                ...
                matchup[(batsman, bowler)]["batsman_team"] = match_details["bat_first"][
                    0
                ]
                matchup[(batsman, bowler)]["bowler_team"] = match_details["bat_first"][
                    1
                ]
            ...
            matchup[(batsman, bowler)]["balls_involved"] += (
            ...
            if "wicket" in ball[delivery]:
                try:
                    if ball[delivery]["wicket"]["player_out"] == batsman:
                        ...
                        if (
                            ball[delivery]["wicket"]["kind"] == "lbw"
                            or ball[delivery]["wicket"]["kind"] == "bowled"
                        ):
        ...
        for ball in innings2:
            for delivery in ball:
                ...
                if (batsman, bowler) not in matchup:
                    ...
                    matchup[(batsman, bowler)]["bowledlbw"] = 0
                    matchup[(batsman, bowler)]["maidens"] = 0
                    matchup[(batsman, bowler)]["batsman_team"] = match_details[
                        "bat_first"
                    ][1]
                    matchup[(batsman, bowler)]["bowler_team"] = match_details[
                        "bat_first"
                    ][0]

                matchup[(batsman, bowler)]["runs"] += ball[delivery]["runs"]["batsman"]
                matchup[(batsman, bowler)]["dots"] += (
                    ball[delivery]["runs"]["total"] == 0
                )
                matchup[(batsman, bowler)]["4s"] += (
                    ball[delivery]["runs"]["batsman"] == 4
                )
                matchup[(batsman, bowler)]["6s"] += (
    ...
    for format in formats:
        ...
        for ele in date_file_pair:
            ...
            for key, headtohead in matchup.items():
                ...
                matchups.loc[count, "strike_rate"] = (
                    headtohead["runs"] * 100 / headtohead["balls"]
                )
                matchups.loc[count, "economy"] = (
                    headtohead["runs"] * 6 / headtohead["balls"]
                )
                ...
                if total_balls[key] == 0:
                    ...
                    matchups.loc[count, "previous_avg_strike_rate"] = (
                        total_runs[key] * 100 / total_balls[key]
                    )
                try:
                    matchups.loc[count, "previous_average"] = (
                        total_runs[key] / wickets[key]
                    )
                except:
                    matchups.loc[count, "previous_average"] = total_runs[key]
                matchups.loc[count, "previous_innings_head_to_head"] = total_innings[
                    key
                ]
                matchups.loc[count, "previous_maidens"] = total_maidens[key]
                matchups.loc[count, "previous_economy"] = (
                    (total_runs[key] * 6 / total_balls[key])
                    if total_balls[key] != 0
                    else 0
                )
        ...
        for i in range(len(matchups)):
            ...
            if not matched.empty:
                matchups.iloc[i, matchups.columns.get_loc("batsman_id")] = matched[
                    "identifier"
                ].values[0]
            else:
            ...
            matched = mapping[mapping["unique_name"] == name]
            if not matched.empty:
                matchups.iloc[i, matchups.columns.get_loc("bowler_id")] = matched[
                    "identifier"
                ].values[0]
            else:
                # print(f"No matching entry for {name}: {len(mapping[mapping['unique_name'] == name])} matches")
