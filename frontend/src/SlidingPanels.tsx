...
import Comparision, { Player } from "./components/comparision_player/Comparision";
...
const SlidingPanels = () => {
  ...
  const extractCardData = (
    ...
    return best11Players?.map((f, index) => {
      ...
      return {
        id: `${index}`, // Unique ID for the player
        name: playerName,
  ...
  const extractRemainingPlayers = (
    ...
    return remainingPlayers.map((playerName: any, index) => {
  ...
  const [dropZoneCards, setDropZoneCards] = useState<CardData[]>(cardData);
