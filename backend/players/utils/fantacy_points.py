...
def calculate_batsman_points_6ixty(runs, boundaries, sixes, strike_rate):
    batsman_points = 0
...
def calculate_batsman_points_hundred(runs, boundaries, sixes, strike_rate):
    batsman_points = 0
    ...
    # 30, 50, and 100 Run Bonus points
    if runs >= 100:
        batsman_points += 20  # 100 runs = 20 points
    elif runs >= 50:
        batsman_points += 10  # 50 runs = 10 points
    elif runs >= 30:
        batsman_points += 5  # 30 runs = 5 points
...
def calculate_bowler_points_6ixty(wickets, bowled_lbw, maidens, economy):
    bowler_points = 0
...
def calculate_bowler_points_hundred(wickets, bowled_lbw, maidens, economy):
    bowler_points = 0
    ...
    # Wicket bonus points
    if wickets == 5:
        bowler_points += 20  # 5 wickets = 20 points
    elif wickets == 4:
        bowler_points += 10  # 4 wickets = 10 points
    elif wickets == 3:
        bowler_points += 5  # 3 wickets = 5 points
    elif wickets == 2:
        bowler_points += 3  # 2 wickets = 3 points
...
def calculate_fielder_points_6ixty(catches, stumpings, runouts_direct, runouts_indirect):
    fielder_points = 0
...
def calculate_fielder_points_hundred(catches, stumpings, runouts_direct, runouts_indirect):
    total_points = 0

    # Points for each catch
    total_points += catches * 8

    # Points for 3 catch bonus
    if catches >= 3:
        total_points += 4

    # Points for stumpings
    total_points += stumpings * 12

    # Points for run outs (direct hit)
    total_points += runouts_direct * 12

    # Points for run outs (not a direct hit)
    total_points += runouts_indirect * 6
...
def points_calculator(format, runs, boundaries, sixes, strike_rate, wickets, bowled_lbw, maidens, economy, catches, stumpings, runouts_direct, runouts_indirect):
    if format == "T20":
        ...
        total_points = (calculate_batsman_points_6ixty(runs, boundaries, sixes, strike_rate) +
                        calculate_bowler_points_6ixty(wickets, bowled_lbw, maidens, economy) +
                        calculate_fielder_points_6ixty(catches, stumpings, runouts_direct, runouts_indirect))

    elif format == "HUNDRED":
        total_points = (calculate_batsman_points_hundred(runs, boundaries, sixes, strike_rate) +
                        calculate_bowler_points_hundred(wickets, bowled_lbw, maidens, economy) +
                        calculate_fielder_points_hundred(catches, stumpings, runouts_direct, runouts_indirect))
